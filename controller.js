...
controller.addMatch = async (req, res) => {
    ...
    if (checkCricketRequiredFileds(body)) {
        return res.status(500).send("Add all required fields for add match.s");
    }
    const AppDataSource = await getDataSource();
    const matchRepo = AppDataSource.getRepository(MatchSchema);

    let alreadyMatchAdded = await matchRepo.findOne({ where: { marketId: body.marketId } });
...
function checkCricketRequiredFileds(body) {
    let cricketRequiredFileds = ['gameType', 'title', 'marketId', 'teamA', 'teamB', 'startAt', 'eventId', 'overType', 'noBallRun']
